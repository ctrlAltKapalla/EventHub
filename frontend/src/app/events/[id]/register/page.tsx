...
export default function RegisterEventPage({ params }: Props) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      setTicketToken(reg.ticketToken);
      setStep("success");
      toast("Registrierung erfolgreich! Dein Ticket wartet.", "success");
    } catch {
      toast("Registrierung fehlgeschlagen. Bitte versuche es erneut.", "error");
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      <div className="max-w-md mx-auto px-4 py-20 text-center">
        ...
        <div className="flex flex-col gap-3">
          <Link href={`/tickets/${ticketToken}`}>
            <Button className="w-full">🎟️ Ticket anzeigen</Button>
          </Link>
          <Link href="/">
            <Button variant="ghost" className="w-full">Weitere Events entdecken</Button>
          </Link>
        </div>
      </div>
    );
        ...
        {(["form", "confirm"] as Step[]).map((s, i) => (
          <div key={s} className="flex items-center gap-2">
            <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold transition-colors ${
              step === s ? "bg-primary-600 text-white" : i < ["form","confirm"].indexOf(step) ? "bg-green-500 text-white" : "bg-neutral-200 text-neutral-500"
            }`}>
              {i < ["form","confirm"].indexOf(step) ? "✓" : i + 1}
            </div>
            <span className={`text-sm ${step === s ? "text-neutral-900 font-medium" : "text-neutral-500"}`}>
              {s === "form" ? "Angaben" : "Bestätigung"}
            </span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2">
          <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-neutral-200 shadow-sm p-8 space-y-5">
            <h1 className="text-xl font-bold text-neutral-900">
              {step === "form" ? "Deine Angaben" : "Angaben bestätigen"}
            </h1>
            ...
            <div className="flex gap-3 pt-2">
              {step === "confirm" && (
                <Button type="button" variant="secondary" onClick={() => setStep("form")}>← Zurück</Button>
              )}
              <Button type="submit" loading={loading} className="flex-1">
                {step === "form" ? "Weiter →" : "Jetzt verbindlich anmelden"}
              </Button>
            </div>
          </form>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-neutral-200 shadow-sm p-6 space-y-4 h-fit">
          ...
          <div>
            ...
            <p className="text-sm text-neutral-600 mt-1">📅 {formattedDate}, {event.time} Uhr</p>
