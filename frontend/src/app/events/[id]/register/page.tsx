...
export default function RegisterEventPage({ params }: Props) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      let token: string;
      if (USE_MOCK) {
        const { addMockRegistration } = await import("@/lib/mockRegistrations");
  ...
  if (!event) {
    return (
      <div className="max-w-7xl mx-auto px-4 py-20 text-center">
        ...
        <Link href="/"><button className="text-primary-600 hover:underline text-sm">Zur Startseite</button></Link>
      </div>
    );
  }
  ...
  if (step === "success") {
    return (
      <div className="max-w-md mx-auto px-4 py-20 text-center">
        ...
        <p className="text-neutral-600 mb-8">Du wirst zu <strong>{event.title}</strong> am {formattedDate} zugelassen.</p>
      ...
      <div className="flex items-center gap-2 mb-8 max-w-lg">
        {(["form", "confirm"] as Step[]).map((s, i) => (
          <div key={s} className="flex items-center gap-2">
            <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold ${step === s ? "bg-primary-600 text-white" : i < ["form","confirm"].indexOf(step) ? "bg-green-500 text-white" : "bg-neutral-200 text-neutral-500"}`}>
              {i < ["form","confirm"].indexOf(step) ? "✓" : i + 1}
            </div>
            <span className={`text-sm ${step === s ? "font-medium text-neutral-900" : "text-neutral-500"}`}>{s === "form" ? "Angaben" : "Bestätigung"}</span>
          ...
          <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-neutral-200 shadow-sm p-8 space-y-5">
            ...
            {step === "form" ? (
              <>
                ...
                <Input label="Telefon (optional)" type="tel" value={form.phone} onChange={(e) => setForm((f) => ({ ...f, phone: e.target.value }))} />
                <Input label="Firma (optional)" value={form.company} onChange={(e) => setForm((f) => ({ ...f, company: e.target.value }))} />
